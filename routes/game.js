...
import { updatePlayerInfoById, getUserById } from "../data/users.js";
          ...
          try {
            ...
            if (Object.keys(user.geocode).length === 0) {
              ...
              return res
                ...
                .render("game_start", {
                  title: "Quiz",
                  error: "You have not provided any location data!",
                  flag,
                });
            }
            ...
            return res
              ...
              .render("game_start", {
                title: "Quiz",
                error:
                  "There aren't enough birds in your location. You can help us By uploading some!",
                flag,
              });
          }
    ...
    if (!req.session.questions || !req.session.timer) {
      ...
      if (questions.length <= index) {
        if (req.session.timer > 0) {
          req.session.score = Math.floor(
            req.session.score * (60 / (60 - req.session.timer))
          );
        }
        res.redirect("/game/gameresult");
      }
      let score = req.session.score;
      return res.render("game_question", {
        title: "Quiz",
...
router.route("/gameresult").get(async (req, res) => {
  try {
    if (req.session.user) {
      let questions = req.session.questions;
      let score = req.session.score;
      ...
      res.render("game_end", {
        title: "Quiz Results",
        score,
        high_score: currentHighScore.toString(),
        lifetime_score: totalLScore.toString(),
      });
    } else {
      let score = req.session.score;
      delete req.session["questions"];
